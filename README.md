# gardener-extension-pcs

The `gardener-extension-pcs` repo provides an implementation of
PodCertificateRequest signer as a Gardener Extension.
...
For local development of the `gardener-extension-pcs` it is recommended that
...
The `deploy` target takes care of the following.
...
``` shell
$ kubectl --kubeconfig $KUBECONFIG_RUNTIME get extop pod-certificate-signer
NAME                         INSTALLED   REQUIRED RUNTIME   REQUIRED VIRTUAL   AGE
pod-certificate-signer       True        False              False              85s
```
...
``` shell
> kubectl --kubeconfig $KUBECONFIG_VIRTUAL get controllerregistrations,controllerdeployments pod-certificate-signer
NAME                                                                    RESOURCES                         AGE
controllerregistration.core.gardener.cloud/pod-certificate-signer       Extension/pod-certificate-signer  3m50s

NAME                                                                  AGE
controllerdeployment.core.gardener.cloud/pod-certificate-signer       3m50s
```
...
`pod-certificate-signer` extension, since it is required by our shoot.
...
pod-certificate-signer-ng4r8       pod-certificate-signer       local   True    True        True      False         2m9s
```
...
pod-certificate-signer   pod-certificate-signer   Succeeded   2m37s
```
...
``` shell
kubectl --kubeconfig $KUBECONFIG_RUNTIME --namespace shoot--local--local annotate extensions pod-certificate-signer gardener.cloud/operation=reconcile
```
...
`gardener-extension-pcs` is hosted on
[Github](https://github.com/gardener/gardener-extension-pcs).
