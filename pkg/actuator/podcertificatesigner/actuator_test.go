...
package podcertificatesigner_test
...
var _ = Describe("Actuator", Ordered, func() {
	var (
		...
		cloudProfileData, seedData, shootData []byte
		...
		actuatorOpts     []podcertificatesigneractuator.Option
	...
	BeforeAll(func() {
		actuatorOpts = []podcertificatesigneractuator.Option{
			podcertificatesigneractuator.WithGardenerVersion("1.0.0"),
			podcertificatesigneractuator.WithDecoder(decoder),
			podcertificatesigneractuator.WithGardenletFeatures(featureGates),
		}
	...
	It("should successfully create an actuator", func() {
		act, err := podcertificatesigneractuator.New(k8sClient, actuatorOpts...)

		Expect(err).NotTo(HaveOccurred())
		Expect(act).NotTo(BeNil())
		Expect(act.Name()).To(Equal(podcertificatesigneractuator.Name))
		Expect(act.ExtensionType()).To(Equal(podcertificatesigneractuator.ExtensionType))
		Expect(act.FinalizerSuffix()).To(Equal(podcertificatesigneractuator.FinalizerSuffix))
	...
	It("should succeed to reconcile without provider config", func() {
		act, err := podcertificatesigneractuator.New(k8sClient, actuatorOpts...)
		Expect(err).NotTo(HaveOccurred())
		Expect(act).NotTo(BeNil())
