...
EXTENSION_NAME ?= gardener-extension-pcs
# Name of the extension resource
EXTENSION_RESOURCE_NAME ?= pod-certificate-signer
...
generate-operator-extension: update-version-tags  ## Generate operator extension example resources.
	@$(GO_TOOL) extension-generator \
		...
		--provider-type pod-certificate-signer \
...
deploy: export IMAGE=$(LOCAL_REGISTRY)/extensions/$(EXTENSION_NAME)

.PHONY: deploy
deploy: update-version-tags docker-build docker-push helm-load-chart  ## Deploy to local dev cluster with Gardener Operator.
	@env \
...
.PHONY: undeploy
undeploy:  ## Cleanup the deployed operator extension.
	@$(GO_TOOL) kustomize build $(SRC_ROOT)/examples/operator-extension | \
